...
// Mock the HomePage component to make testing easier
...
describe("Routes in MainContent", () => {
  ...
  it("renders HomePage on catch-all route (*)", () => {
    render(
    ...
    // Check that HomePage is rendered for unknown routes
    expect(screen.getByTestId("home-page")).toBeInTheDocument();
    expect(screen.getByText("Home Page")).toBeInTheDocument();
  });

  it("renders HomePage on any other route", () => {
    const testRoutes = ["/about", "/contact", "/dashboard", "/settings"];
    ...
    testRoutes.forEach((route) => {
      ...
      // Check that HomePage is rendered for any route
      expect(screen.getByTestId("home-page")).toBeInTheDocument();
      expect(screen.getByText("Home Page")).toBeInTheDocument();
  ...
  it("maintains proper DOM structure", () => {
    ...
    const routesContainer = container?.firstChild;
    const homePage = screen.getByTestId("home-page");

    expect(mainElement).toContainElement(container as Element);
    expect(container).toContainElement(routesContainer as Element);
    expect(routesContainer).toContainElement(homePage);
  });
});
